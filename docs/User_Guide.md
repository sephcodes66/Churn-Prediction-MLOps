...
**Goal**: Validate data integrity and schema
...
features = build_features(df.copy(), is_training_data=True)
...
df_processed = build_features(df.copy(), is_training_data=False)
...
# Or run each phase individually (recommended for learning)
python -m src.ingest_data
python -m src.data_quality_checks
python -m src.data_splitter
python -m src.train
...
python -m src.monitor_performance
