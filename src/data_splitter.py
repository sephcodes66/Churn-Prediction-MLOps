"""
IMPROVED MLOps Phase 2: Data Splitting for Time Series

This module implements proper data splitting for time series data following
best practices to prevent data leakage and ensure temporal consistency.

Key Improvements:
- Proper time-series splitting (no unit mixing between splits)
- Temporal consistency validation
- Balanced distribution across splits
- Comprehensive split validation and reporting
...
class TimeSerisSplitter:
    """Time series data splitter with proper validation"""
    ...
    def _create_split_config(self) -> SplitConfig:
        """Create split configuration with validation"""
    ...
    def analyze_data_distribution(self, df: pd.DataFrame) -> Dict:
        """Analyze data distribution for informed splitting"""
        analysis = {}
        
        # Unit-level analysis
        unit_stats = df.groupby('unit_number').agg({
            'time_in_cycles': ['count', 'max'],
            'sensor_1': ['mean', 'std']  # Representative sensor
        }).round(2)
        ...
        # Filter units based on lifecycle criteria
    ...
    def create_balanced_splits(self, df: pd.DataFrame, valid_units: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """Create balanced splits ensuring no data leakage"""
        
        # Sort units by lifecycle length for balanced distribution
        ...
        # Distribute units proportionally across quartiles
        ...
        for quartile in ['Q1', 'Q2', 'Q3', 'Q4']:
            ...
            # Calculate proportional sizes for this quartile
    ...
    def _validate_no_overlap(self, train_units: List, val_units: List, test_units: List):
        """Validate no unit overlap between splits"""
    ...
    def validate_temporal_consistency(self, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame) -> Dict:
        """Validate temporal consistency within each split"""
        validation_results = {}
        ...
        for name, df in [('train', train_df), ('val', val_df), ('test', test_df)]:
            ...
            for unit in df['unit_number'].unique():
                ...
                # Check for consecutive time cycles starting from 1
    ...
    def analyze_split_balance(self, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame) -> Dict:
        ...
        # Statistical tests could be added here
    ...
    def save_splits(self, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame) -> bool:
        """Save split data with validation"""
        try:
    ...
    def create_split_visualization(self, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame):
        """Create visualization of split characteristics"""
...
def main():
    """Main function to run improved data splitting"""
